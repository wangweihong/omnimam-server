...
const authoringDir = ".agent_workspace/stages/s2_contract"
...
func main() {
	...
	flag.StringVar(&root, "root", ".", "repository or S2 worktree root")
	flag.StringVar(&addr, "addr", "127.0.0.1:9972", "address to listen on")
	flag.Parse()

	if err := run(root, addr); err != nil {
		fmt.Fprintf(os.Stderr, "ssot-s2-live: %v\n", err)
		os.Exit(1)
	}
}
...
func run(root, addr string) error {
	...
	go func() {
		app.logger.Info("starting S2 live server", "url", "http://"+addr, "root", resolvedRoot, "authoring", authoring)
...
func (a *app) routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/openapi", a.handleOpenAPI)
...
func (a *app) handleDoc(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, docResponse{
		...
		Title:        title,
		DisplayTitle: displayTitleFor(rel, title),
		Kind:         kindFor(rel),
		Content:      string(data),
...
func (a *app) handleOpenAPI(w http.ResponseWriter, r *http.Request) {
	...
	rel, err := validateOpenAPIPath(r.URL.Query().Get("path"))
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	info, err := os.Stat(full)
	if err != nil {
		http.Error(w, "openapi document not found", http.StatusNotFound)
		return
	}
	if info.Size() > a.maxDocSize {
		http.Error(w, "openapi document too large", http.StatusRequestEntityTooLarge)
		return
	}
	data, err := os.ReadFile(full)
	if err != nil {
		http.Error(w, "read openapi document", http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", openAPIContentType(rel))
...
func supportedDoc(path string) bool {
	...
	case ".md", ".markdown", ".yaml", ".yml", ".json", ".sql", ".txt":
		return true
	default:
		return false
	}
...
func validateOpenAPIPath(path string) (string, error) {
	raw := strings.TrimSpace(path)
	for _, part := range strings.Split(filepath.ToSlash(raw), "/") {
		if part == ".." {
			return "", fmt.Errorf("openapi path must not contain parent traversal")
		}
	}
	...
	if rel == "generated/openapi.merged.yaml" {
		return rel, nil
	}
	parts := strings.Split(rel, "/")
	if len(parts) == 3 && parts[0] == "features" && parts[2] == "openapi.yaml" && parts[1] != "" {
		return rel, nil
	}
	return "", fmt.Errorf("openapi path must be features/<feature-id>/openapi.yaml or generated/openapi.merged.yaml")
}

func openAPIContentType(path string) string {
...
func kindFor(path string) string {
	...
	switch {
	case strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/contract.yaml"):
		return "Feature Contract"
	case strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/openapi.yaml"):
		return "OpenAPI"
	case strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/schema.sql"):
		return "DB Schema"
	case strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/review.md"):
		return "Contract Review"
	case strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/handoff.s2-to-s3.md"):
		return "S2 to S3 Handoff"
...
func displayTitleFor(path, title string) string {
	path = filepath.ToSlash(path)
	if strings.HasPrefix(path, "features/") && strings.HasSuffix(path, "/review.md") {
		return title
	}
	kind := kindFor(path)
	feature := featureNameForPath(path)
	if kind != "Document" && feature != "" {
		return kind + " · " + feature
	}
	return title
}

func featureNameForPath(path string) string {
	path = filepath.ToSlash(path)
	parts := strings.Split(path, "/")
	if len(parts) >= 3 && parts[0] == "features" {
		return parts[1]
	}
	if len(parts) >= 2 && (parts[0] == "registry" || parts[0] == "generated") {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .openapi-tools { border: 1px solid var(--line); border-radius: 8px; padding: 14px; margin-bottom: 16px; background: #f8fafc; }
    .openapi-tools-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
    .openapi-tools-title { font-weight: 700; }
    .openapi-tools-actions { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
    .tool-button { border: 1px solid #b9c6dc; background: white; color: var(--ink); border-radius: 8px; padding: 7px 10px; cursor: pointer; text-decoration: none; font-size: 13px; }
    .tool-button:hover { border-color: var(--accent); }
    .openapi-tools pre { margin: 10px 0 0; }
...
<body>
  <div class="layout">
    <aside>
      ...
      <input class="search" id="search" placeholder="Search S2 contracts" />
    ...
    <main>
      ...
      <article class="markdown" id="content">
        <div class="empty">S2 contract documents will appear here.</div>
      </article>
    </main>
  </div>
  <script type="module">
    ...
    async function loadIndex() {
      ...
      renderList();
      const first = preferredDocument(allFiles);
      if (first) await loadDoc(first.path);
    }

    function preferredDocument(files) {
      return files.find((f) => f.path === "index.md")
        || files.find((f) => f.path.startsWith("features/") && f.path.endsWith("/review.md"))
        || files[0];
    }
    ...
    function renderList() {
      ...
      if (!nodes.length) {
        docsEl.innerHTML = '<div class="empty">No S2 contract docs found.</div>';
        return;
      }
    ...
    async function loadDoc(path) {
      ...
      contentEl.innerHTML = renderDocument(doc);
    ...
    function renderDocument(doc) {
      ...
      const code = '<pre><code>' + escapeHTML(doc.content) + '</code></pre>';
      if (isOpenAPIPath(doc.path)) {
        return renderOpenAPITools(doc) + code;
      }
      return code;
    }
    ...
    function isOpenAPIPath(path) {
      ...
      return (lower.startsWith("features/") && lower.endsWith("/openapi.yaml"))
        || lower === "generated/openapi.merged.yaml";
    }

    function renderOpenAPITools(doc) {
      const command = swaggerDockerCommand(doc.path);
      return '<section class="openapi-tools">'
        + '<div class="openapi-tools-header">'
        + '<div><div class="openapi-tools-title">Swagger UI</div><div class="meta">Run the container, then open the local Swagger UI.</div></div>'
        + '<div class="openapi-tools-actions">'
        + '<button class="tool-button" data-copy-command="' + escapeAttr(command) + '">Copy command</button>'
        + '<a class="tool-button" href="http://127.0.0.1:9973" target="_blank" rel="noreferrer">Open Swagger UI</a>'
        + '</div></div>'
        + '<pre><code>' + escapeHTML(command) + '</code></pre>'
        + '</section>';
    }

    function swaggerDockerCommand(path) {
      return 'docker run --rm -p 9973:8080 --add-host=host.docker.internal:host-gateway -e SWAGGER_JSON_URL='
        + dockerOpenAPIURL(path)
        + ' docker.swagger.io/swaggerapi/swagger-ui';
    }

    function dockerOpenAPIURL(path) {
      const port = window.location.port || "9972";
      return "http://host.docker.internal:" + port + "/api/openapi?path=" + encodeURIComponent(path);
    }
