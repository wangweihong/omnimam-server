...
func underlyingType(t *types.Type) *types.Type {
	// Old gengo versions may create incomplete Alias nodes for syntax they do
	// not fully understand. Keep this nil-safe and cycle-safe because `any` is
	// represented through exactly that path on Go 1.18+ source.
	seen := map[*types.Type]bool{}
	for t != nil && t.Kind == types.Alias && t.Underlying != nil && !seen[t] {
		seen[t] = true
		t = t.Underlying
	}
	return t
...
// isUnnameableUnsupported reports a type node that gengo's raw namer cannot
// emit as valid Go source. With older gengo releases, the predeclared alias
// `any` becomes types.Unsupported and is rendered as unnameable_Unsupported.
//
// Do not depend on Name.Name == "any": when `any` is nested inside a composite
// such as []map[string]any, some parser versions retain only the Unsupported
// kind or the generated unnameable marker.
func isUnnameableUnsupported(t *types.Type) bool {
	if t == nil {
		return false
	}
	...
	if strings.Contains(name, "unnameable_unsupported") {
		return true
	}

	text := strings.ToLower(t.String())
	return strings.Contains(text, "unnameable_unsupported")
}

// isEmptyInterface reports whether t is interface{} / any (including aliases).
func isEmptyInterface(t *types.Type) bool {
	...
	if t.Kind == types.Interface && len(t.Methods) == 0 {
		return true
	}

	// Preserve the specific any/interface{} check for places where the generated
	// expression itself is interface{}-typed. Composite handling uses the broader
	// isUnnameableUnsupported check below.
	for _, candidate := range []string{original.Name.Name, t.Name.Name, original.String(), t.String()} {
		candidate = strings.ToLower(strings.TrimSpace(candidate))
		if candidate == "any" || candidate == "interface{}" || candidate == "interface {}" ||
			strings.Contains(candidate, "unnameable_unsupported") {
			return true
		}
	}
	return false
...
// containsEmptyInterfaceType reports whether emitting t through gengo's raw
// namer can produce an invalid type expression. The name is retained to keep the
// call sites focused, but the check intentionally accepts every Unsupported
// leaf, not only a leaf whose name is still exactly "any".
//
// This is required for []map[string]any: depending on the gengo version, the
// nested leaf may have Kind Unsupported with an empty/rewritten name, so checking
// only Name.Name == "any" misses it and produces []map[string]unnameable_Unsupported.
func containsEmptyInterfaceType(t *types.Type) bool {
	return containsEmptyInterfaceTypeSeen(t, map[*types.Type]bool{})
}

func containsEmptyInterfaceTypeSeen(t *types.Type, seen map[*types.Type]bool) bool {
	if t == nil {
		return false
	}
	if seen[t] {
	...
	if isEmptyInterface(t) || isUnnameableUnsupported(t) {
		return true
	}
	...
	if ut != t && (isEmptyInterface(ut) || isUnnameableUnsupported(ut)) {
		return true
	}

	switch ut.Kind {
	case types.Map:
		return containsEmptyInterfaceTypeSeen(ut.Key, seen) || containsEmptyInterfaceTypeSeen(ut.Elem, seen)
	case types.Slice, types.Pointer, types.Array:
		return containsEmptyInterfaceTypeSeen(ut.Elem, seen)
	case types.Struct:
		if !ut.IsAnonymousStruct() {
			return false
		}
		for _, member := range ut.Members {
			if containsEmptyInterfaceTypeSeen(member.Type, seen) {
				return true
			}
		}
	}
...
// deepCopyAnyValueFunc marks the generated file as requiring the dynamic-value
// deepcopy helper and returns the helper function name.
func (g *genDeepCopy) deepCopyAnyValueFunc() string {
	g.needsAnyDeepCopy = true
	g.imports.AddSymbol(types.Name{Package: "reflect", Name: "Value"})
	return "deepcopyGenAnyValue"
}

// deepCopyAnyIntoFunc marks the generated file as requiring the reflection
// helper used to copy a complete typed value without spelling its type. This is
// required for nested composites such as []map[string]any, whose raw gengo name
// contains unnameable_Unsupported.
func (g *genDeepCopy) deepCopyAnyIntoFunc() string {
	g.deepCopyAnyValueFunc()
	return "deepcopyGenAnyInto"
}
...
func (g *genDeepCopy) Finalize(c *generator.Context, w io.Writer) error {
	if !g.needsAnyDeepCopy {
		return nil
	}

	reflectPackage := g.imports.LocalNameOf("reflect")
...
`, "$reflect$", reflectPackage)

	_, err := io.WriteString(w, helper)
	return err
}
...
func (g *genDeepCopy) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	// Do not render a composite map type containing any. For example,
	// map[string][]map[string]any would otherwise contain
	// unnameable_Unsupported in the generated source.
	if containsEmptyInterfaceType(t) {
		sw.Do(fmt.Sprintf("%s(in, out)\n", g.deepCopyAnyIntoFunc()), nil)
		return
	}
	...
	// Older gengo versions render the predeclared alias `any` as
	// `unnameable_Unsupported`. Do not ask the raw namer to print a map whose
	// element is any; interface{} is identical to any and is always nameable.
	if isEmptyInterface(uet) {
		sw.Do("*out = make(map[$.Key|raw$]interface{}, len(*in))\n", ut)
	} else {
	...
	case ut.Elem.IsAnonymousStruct(): // not uet here because it needs type cast
		sw.Do("(*out)[key] = val\n", nil)
	case isEmptyInterface(uet):
		sw.Do(fmt.Sprintf("(*out)[key] = %s(val)\n", g.deepCopyAnyValueFunc()), nil)
...
func (g *genDeepCopy) doSlice(t *types.Type, sw *generator.SnippetWriter) {
	...
	// Copy the complete slice through reflection when any appears at any nested
	// level. This covers []any, []map[string]any, [][]any, and deeper mixtures
	// without asking the raw namer to spell the composite type.
	if containsEmptyInterfaceType(t) {
		sw.Do(fmt.Sprintf("%s(in, out)\n", g.deepCopyAnyIntoFunc()), nil)
		return
	}

	// As with maps, old gengo cannot render []any because `any` is represented
	// as types.Unsupported. Emit []interface{} for the allocation instead.
	if isEmptyInterface(uet) {
		sw.Do("*out = make([]interface{}, len(*in))\n", nil)
	} else {
		sw.Do("*out = make($.|raw$, len(*in))\n", t)
	}
	if deepCopyMethodOrDie(ut.Elem) != nil || deepCopyIntoMethodOrDie(ut.Elem) != nil {
		...
		sw.Do("}\n", nil)
	} else if isEmptyInterface(uet) {
		sw.Do("for i := range *in {\n", nil)
		sw.Do(fmt.Sprintf("(*out)[i] = %s((*in)[i])\n", g.deepCopyAnyValueFunc()), nil)
		sw.Do("}\n", nil)
...
func (g *genDeepCopy) doStruct(t *types.Type, sw *generator.SnippetWriter) {
	...
	for _, m := range ut.Members {
		...
		case containsEmptyInterfaceType(ft):
			sw.Do(fmt.Sprintf("%s(&in.$.name$, &out.$.name$)\n", g.deepCopyAnyIntoFunc()), args)
		...
		case isEmptyInterface(uft):
			sw.Do(fmt.Sprintf("out.$.name$ = %s(in.$.name$)\n", g.deepCopyAnyValueFunc()), args)
...
func (g *genDeepCopy) doPointer(t *types.Type, sw *generator.SnippetWriter) {
	...
	dc, dci := deepCopyMethodOrDie(ut.Elem), deepCopyIntoMethodOrDie(ut.Elem)
	if dc == nil && dci == nil && containsEmptyInterfaceType(t) {
		sw.Do(fmt.Sprintf("%s(in, out)\n", g.deepCopyAnyIntoFunc()), nil)
		return
	}
	...
	case uet.Kind == types.Map, uet.Kind == types.Slice, uet.Kind == types.Pointer:
		...
		sw.Do("}\n", nil)
	case isEmptyInterface(uet):
		// Avoid rendering *any through the raw namer. The helper returns
		// interface{}, which is identical to any, so taking its address produces
		// the correct pointer type without spelling `any`.
		sw.Do(fmt.Sprintf("x := %s(**in)\n", g.deepCopyAnyValueFunc()), nil)
		sw.Do("*out = &x\n", nil)
	case uet.Kind == types.Struct:
		sw.Do("*out = new($.Elem|raw$)\n", ut)
		sw.Do("(*in).DeepCopyInto(*out)\n", nil)
