...
func (Adapter) Check(ctx context.Context, engine *iapiserver.EngineInstance) (*iapiserver.EngineHealthCheckResult, error) {
	if err := provider.Probe(ctx, engine, http.MethodGet, "/", provider.WithAPIKeyInPayload()); err != nil {
		return nil, err
	}
...
func (Executor) ExecuteCheckpoint(ctx context.Context, engine *iapiserver.EngineInstance, run *iapiserver.ApplicationRun, checkpoint map[string]any) (map[string]any, error) {
	...
	if taskID == "" {
		...
		submitted, err := provider.Invoke(ctx, engine, http.MethodPost, "/task/openapi/create", payload, provider.WithAPIKeyInPayload())
		if err != nil {
			return nil, err
		}
	...
	result, err := provider.Invoke(ctx, engine, http.MethodPost, "/task/openapi/outputs", map[string]any{"apiKey": apiKey, "taskId": taskID}, provider.WithAPIKeyInPayload())
	if err != nil {
		if ctx.Err() != nil {
			return nil, stderrors.Join(ctx.Err(), cancelTask(ctx, engine, apiKey, taskID))
		}
...
func (Executor) Execute(ctx context.Context, engine *iapiserver.EngineInstance, run *iapiserver.ApplicationRun) (map[string]any, error) {
	...
	for {
		...
		case <-ctx.Done():
			return nil, stderrors.Join(ctx.Err(), cancelTask(ctx, engine, apiKey, taskID))
		case <-ticker.C:
			result, pollErr := provider.Invoke(ctx, engine, http.MethodPost, "/task/openapi/outputs", map[string]any{"apiKey": apiKey, "taskId": taskID}, provider.WithAPIKeyInPayload())
			if pollErr != nil {
				if ctx.Err() != nil {
					return nil, stderrors.Join(ctx.Err(), cancelTask(ctx, engine, apiKey, taskID))
				}
...
func cancelTask(ctx context.Context, engine *iapiserver.EngineInstance, apiKey, taskID string) error {
	...
	defer cancel()
	_, err := provider.Invoke(cancelCtx, engine, http.MethodPost, "/task/openapi/cancel", map[string]any{"apiKey": apiKey, "taskId": taskID}, provider.WithAPIKeyInPayload())
	return err
}
