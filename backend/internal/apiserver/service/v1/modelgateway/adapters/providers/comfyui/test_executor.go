...
func (e *TestExecutor) Submit(ctx context.Context, testRunID string) (map[string]any, error) {
	...
	result, err := httpjson.Invoke(ctx, engine, http.MethodPost, "/prompt", map[string]any{"prompt": workflow, "client_id": run.ID}, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return nil, err
	}
...
func (e *TestExecutor) Poll(ctx context.Context, testRunID string) (map[string]any, error) {
	...
	history, err := httpjson.Invoke(ctx, engine, http.MethodGet, "/history/"+url.PathEscape(promptID), nil, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return nil, err
	}
...
func (e *TestExecutor) Collect(ctx context.Context, testRunID string) (map[string]any, error) {
	...
	history, err := httpjson.Invoke(ctx, engine, http.MethodGet, "/history/"+url.PathEscape(*run.ExternalJobID), nil, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return nil, err
	}
...
func comfyQueuePosition(ctx context.Context, engine *iapiserver.EngineInstance, promptID string) *int {
	queue, err := httpjson.Invoke(ctx, engine, http.MethodGet, "/queue", nil, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return nil
	}
	position := 0
