...
func (Adapter) Check(ctx context.Context, engine *iapiserver.EngineInstance) (*iapiserver.EngineHealthCheckResult, error) {
	if err := httpjson.Probe(ctx, engine, http.MethodGet, "/system_stats", httpjson.WithAPIKeyHeader("X-API-Key")); err != nil {
		return nil, err
	}
...
func (Adapter) ReadObjectInfo(ctx context.Context, engine *iapiserver.EngineInstance) (map[string]any, error) {
	result, err := httpjson.Invoke(ctx, engine, http.MethodGet, "/object_info", nil, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
...
func (Adapter) ReadComfyUIVersion(ctx context.Context, engine *iapiserver.EngineInstance) (string, error) {
	result, err := httpjson.Invoke(ctx, engine, http.MethodGet, "/system_stats", nil, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return "", err
	}
...
func (Executor) Execute(ctx context.Context, engine *iapiserver.EngineInstance, run *iapiserver.ApplicationRun) (map[string]any, error) {
	...
	submitted, err := httpjson.Invoke(ctx, engine, http.MethodPost, "/prompt", map[string]any{"prompt": workflow, "client_id": run.ID}, httpjson.WithAPIKeyHeader("X-API-Key"))
	if err != nil {
		return nil, err
	}
	...
	for {
		select {
		case <-ctx.Done():
			_, cancelErr := httpjson.Invoke(context.WithoutCancel(ctx), engine, http.MethodPost, "/interrupt", map[string]any{}, httpjson.WithAPIKeyHeader("X-API-Key"))
			return nil, stderrors.Join(ctx.Err(), cancelErr)
		case <-ticker.C:
			history, pollErr := httpjson.Invoke(ctx, engine, http.MethodGet, "/history/"+url.PathEscape(promptID), nil, httpjson.WithAPIKeyHeader("X-API-Key"))
			if pollErr != nil {
				return nil, pollErr
			}
