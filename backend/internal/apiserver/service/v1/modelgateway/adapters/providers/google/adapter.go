...
func (Adapter) Check(ctx context.Context, engine *iapiserver.EngineInstance) (*iapiserver.EngineHealthCheckResult, error) {
	if err := httpjson.Probe(ctx, engine, http.MethodGet, "/models", httpjson.WithAPIKeyHeader("x-goog-api-key")); err != nil {
		return nil, err
	}
...
func (Executor) Execute(ctx context.Context, engine *iapiserver.EngineInstance, run *iapiserver.ApplicationRun) (map[string]any, error) {
	...
	response, err := httpjson.Invoke(ctx, engine, http.MethodPost, "/interactions", payload, httpjson.WithAPIKeyHeader("x-goog-api-key"))
	if err != nil {
		return nil, err
	}
