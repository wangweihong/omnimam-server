...
func (Adapter) Check(ctx context.Context, engine *iapiserver.EngineInstance) (*iapiserver.EngineHealthCheckResult, error) {
	if err := httpjson.Probe(ctx, engine, http.MethodGet, "/api/v3/contents/generations/tasks", httpjson.WithModelArkSigning()); err != nil {
		return nil, err
	}
...
func (e *Executor) Execute(ctx context.Context, engine *iapiserver.EngineInstance, run *iapiserver.ApplicationRun) (map[string]any, error) {
	...
	submitted, err := httpjson.Invoke(ctx, engine, http.MethodPost, requestPath, payload, httpjson.WithModelArkSigning())
	if err != nil {
		return nil, err
	}
	...
	for {
		select {
		case <-ctx.Done():
			_, cancelErr := httpjson.Invoke(context.WithoutCancel(ctx), engine, http.MethodDelete, requestPath+"/"+url.PathEscape(taskID), nil, httpjson.WithModelArkSigning())
			return nil, stderrors.Join(ctx.Err(), cancelErr)
		case <-ticker.C:
			result, pollErr := httpjson.Invoke(ctx, engine, http.MethodGet, requestPath+"/"+url.PathEscape(taskID), nil, httpjson.WithModelArkSigning())
			if pollErr != nil {
				return nil, pollErr
			}
