...
func Registration() (modelgateway.Registration, error) {
	capabilities, err := modelgateway.ParseProviderCapabilityManifest("runninghub", capabilityManifestYAML)
	if err != nil {
	...
	return modelgateway.Registration{
		...
		EngineType:            iapiserver.ApplicationEngineType{ID: AdapterID, NameI18n: bilingual("RunningHub 工作流", "RunningHub Workflow"), DescriptionI18n: bilingual("RunningHub 托管 ComfyUI 工作流异步执行服务。", "RunningHub hosted asynchronous ComfyUI workflow service."), OfficialWebsiteURL: "https://www.runninghub.cn/", OfficialDocumentationURL: "https://www.runninghub.cn/runninghub-api-doc-cn", DefaultAPIBaseURL: "https://www.runninghub.cn", Enabled: true, EngineAdapterID: AdapterID, AuthenticationTypes: []string{iapiserver.EngineAuthAPIKey}, AuthenticationConfigSchema: modelgateway.AuthenticationConfigSchema(iapiserver.EngineAuthAPIKey), OperationExecutors: map[string]string{definition.ID: ExecutorID}},
