...
var catalog = map[string]definition{
	...
	RepresentationInspect:  {names: names("检查素材视图", "Inspect AssetVersion representations")},
	RepresentationGenerate: {params: []string{"representation_type"}, names: names("生成 {{representation_type}}视图", "Generate {{representation_type}}")},
	RepresentationFinalize: {names: names("完成素材视图构建", "Finalize AssetVersion representations")},
	RepresentationBuild:    {names: names("构建素材视图", "Build AssetVersion representations")},
	RepresentationBackfill: {names: names("每日补全素材视图", "Daily AssetVersion representation backfill")},
	ComfyUIWorkflowTest:    {names: names("ComfyUI工作流试运行", "ComfyUI workflow test")},
	...
	ObjectInfoRefresh:      {names: names("每日刷新ComfyUI节点信息", "Daily ComfyUI object_info refresh")},
	BackfillThumbnail:      {names: names("补全素材缩略图", "Backfill AssetVersion thumbnail")},
}
