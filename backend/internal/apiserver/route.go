...
func installApis(
	...
	if storeIns != nil {
		if appStudio != nil {
			...
			g.GET(mcpprotocol.AppStudioWorkspaceToolPath, appstudioctrl.NewController(appStudio).HandleWorkspaceTool)
		}
...
func installAppStudioApis(rg *gin.RouterGroup, service *appstudiosvc.Service) {
	...
	agentRead.GET("/runtime", c.GetAgentRuntime)
	agentRead.GET("/runtimes", c.ListAgentRuntimes)
	agentRead.GET("/runtime/health", c.GetAgentRuntimeHealth)
	...
	runtimeLogsRead := rg.Group("/studio-applications/:studio_application_id/agent/runtime/logs")
	runtimeLogsRead.Use(authmiddleware.RequireIdentityPermission("appstudio.agent.runtime.logs.read"))
	runtimeLogsRead.GET("", c.ListAgentRuntimeLogs)
