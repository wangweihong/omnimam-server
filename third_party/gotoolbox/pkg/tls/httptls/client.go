...
func NewClientCredentials(config *TlsConfig)(*tls.Config, error) {
	if config == nil || config.CAData == ""  {
		return NewTlsClientSkipVerifiedCredentials(),nil
	}

	if config.KeyData == ""  || config.CertData == ""{
		return NewTlsClientCredentials([]byte(config.CAData))
	}

	return NewMutualTlsClientCredentials([]byte(config.CAData), []byte(config.CertData), []byte(config.KeyData))
}
