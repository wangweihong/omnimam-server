package tokenutil_test

import (
	"crypto/x509"
	"encoding/pem"
	"net/url"
	"testing"
	"time"

	. "github.com/smartystreets/goconvey/convey"

	"github.com/wangweihong/gotoolbox/pkg/tokenutil"
)

func TestOtt(t *testing.T) {
	key := `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
	Convey("TestOTT", t, func() {
		baseURL, err := url.Parse("192.168.134.187")
		So(err, ShouldBeNil)
		b, _ := pem.Decode([]byte(key))
		k, _ := x509.ParsePKCS1PrivateKey(b.Bytes)
		m := tokenutil.NewOneTimeTokenManager(tokenutil.JWTTrackedRequestCodec{
			SigningMethod: tokenutil.DefaultJWTSigningMethod,
			Audience:      baseURL.String(),
			Issuer:        baseURL.String(),
			MaxAge:        3 * time.Second,
			Key:           k,
		})
		ts := "test"
		signed, err := m.Put(ts)
		So(err, ShouldBeNil)
		So(signed, ShouldNotEqual, "")

		tr, err := m.Get(signed)
		So(err, ShouldBeNil)

		_, err = m.Get(signed)
		So(err, ShouldNotBeNil)
		So(err.Error(), ShouldEqual, "signed token not exist")
		So(tr.Value, ShouldResemble, ts)

		signed, err = m.Put(ts)
		So(err, ShouldBeNil)

		time.Sleep(5 * time.Second)
		_, err = m.Get(signed)
		So(err, ShouldNotBeNil)
		So(err.Error(), ShouldContainSubstring, "token is expired by")

	})

}
